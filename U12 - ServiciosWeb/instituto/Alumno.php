...
public static function getAlumnosByCurso($curso){
    $conexion = InstitutoDB::connectDB();
    $seleccion = "SELECT matricula, nombre, apellidos, curso FROM alumno WHERE curso='$curso'";
    $consulta = $conexion->query($seleccion);
...
public static function getAlumnosByNombre($nombre) {
    $alumnosAux = Alumno::getAlumnos();
    $alumnos = [];
    
    foreach ($alumnosAux as $alumno) {
        if ( str_contains(strtolower($alumno->nombre),strtolower($nombre)) ) {
            $alumnos[]=$alumno;
        }
    }
    return $alumnos;
